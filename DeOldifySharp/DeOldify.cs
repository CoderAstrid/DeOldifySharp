...
namespace ColorfulSoft.DeOldify
{
    ...
    internal static unsafe class DeOldify
    {
        ...
        {
            var mePath = Path.GetDirectoryName(Assembly.GetEntryAssembly().Location) + "\\";
#if half
    #if stable
            mePath += "Stable.hmodel";
    #else
            mePath += "Artistic.hmodel";
    #endif
#else
    #if stable
             mePath += "Stable.model";
    #else
             mePath += "Artistic.model";
    #endif
#endif
            using (Stream src = File.OpenRead(mePath))
            {
